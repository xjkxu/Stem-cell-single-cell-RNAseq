...
} else{genes_cords <- as.data.frame(wb[[geneset+1]])}
# Create a dataframe
...
read_and_process_file <- function(r,results_dir) {
  ...
  #tmp <- fread(paste0(results_dir, "/ColocalizedMarkers/reml_", r, ".vars"), fill = TRUE)
  
  #return(data.frame(trait = r, h2 = as.numeric(tmp[1, 2])))
...
genesets <- c(1, 2, 3, 4, "Combined")
# Iterate over each geneset
for(r in c("Genic","2kb")){
  h2.dir <- paste0("h2_analysis/",r,"/")
  h2_dataframes <- list()
...
process_permutations <- function(results_dir, t, num_permutations) {
  ...
  process_permutation <- function(r, results_dir, t) {
    if (file.exists(paste0(results_dir, "/Permutations/", t, "/reml", r, ".reml"))) {
      ...
      #h2_value <-as.numeric(tmp[1, 2])
      
      #if h2
      #tmp <- fread(paste0(results_dir, "/Permutations/", t, "/reml", r, ".reml"), fill = TRUE)
      #return(data.frame(trait = r, h2 = as.numeric(tmp[18, 2])))
      tmp <- fread(paste0(results_dir, "/Permutations/", t, "/reml", r, ".reml"), fill = TRUE)[13:20, ]
...
gene_space <- c("Genic","2kb")
 for(gs in gene_space){
   h2.dir <- paste0("h2_analysis/",gs,"/")
  for(g in genesets){
    print(paste(gs,g))
results.dir <- paste0(h2.dir,"GeneSet",g)
h2_perm_list <- lapply(traits$full_trait_names, function(t) {
  process_permutations(results.dir, t, 1000)
})
h2_perm_df <- do.call(cbind, h2_perm_list)
colnames(h2_perm_df) <- traits$full_trait_names
...
permutations <- merge(permutations,traits,by.x="variable",by.y="full_trait_names",all.x=T)
combined_h2_melt <- melt(as.data.table(combined_h2))
combined_h2_melt$regions <- factor(combined_h2_melt$regions)
permutations$ID <- paste(permutations$traits,permutations$genespace,permutations$geneset,sep="_")
...
save(permutations,file="StemCellGeneSeth2Permuations.Rdata")

#get percentiles
ID <- unique(combined_h2_melt$ID)[1]
...
for(i in genesets){
 ...
 permutations_tmp <-  permutations_tmp[order(permutations_tmp$traits, permutations_tmp$region), ]
  ...
  # Reorder the dataframe based on the order of the matching vector
  if(i != "Combined"){
    title <- names(wb)[1+which(genesets==i)]
  } else if (i == "Combined"){
